...
- Before adding new function the agent has to check this function is not yet available as another function which could be reused or extended. Do not duplicate logic if possible.
  - Before creating new frontend components check that you cannot reuse a component or composable from [`@nextcloud/vue`](https://nextcloud-vue-components.netlify.app/#/Introduction).
  Before creating new backend logic check for available functions and classes provided by Nextcloud in the `OCP` namespace.
- When adding new frontend features they have to be covered with end-to-end tests
  - End-to-end tests are using playwright, follow playwright best practice. Focus on non flaky tests. Use Page Object Models and reuse fixtures. Use accessible selectors (accessible name or role) instead of data-* or class name locators. 
- Frontend logic changes should be covered by vitest unit tests, non-component changes must be covered with those unit tests.
- Backend changes require test coverage of the changes using the PHPUnit tests.
