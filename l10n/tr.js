OC.L10N.register(
    "end_to_end_encryption",
    {
    "Internal error" : "İçeride bir sorun çıktı",
    "You are not allowed to delete this private key" : "Bu kişisel anahtarı silme izniniz yok",
    "Could not find the private key belonging to the user %s" : "%s kullanıcısına ait kişisel anahtar bulunamadı",
    "Could not parse the CSR, please make sure to submit a valid CSR" : "Sertifika imzalama isteği işlenemedi. Lütfen geçerli bir sertifika imzalama isteği gönderdiğinizden emin olun",
    "Common name (CN) does not match the share token" : "Ortak ad (CN) paylaşım kodu ile uyuşmuyor",
    "You are not the owner of the share" : "Paylaşımın sahibi değilsiniz",
    "Common name (CN) does not match the current user" : "Ortak ad (CN) geçerli kullanıcı ile uyuşmuyor",
    "Could not find the public key belonging to %s" : "%s kullanıcısına ait herkese açık anahtar bulunamadı",
    "This is not your public key to delete" : "Silinecek herkese açık anahtar sizin değil",
    "Cannot decode userlist" : "Kullanıcı listesinin kodu çözülemedi",
    "Could not find metadata for \"%s\"" : "\"%s\" üst verisi bulunamadı",
    "Cannot read metadata" : "Üst veri okunamadı",
    "Cannot store metadata" : "Üst veri kaydedilemedi",
    "Metadata-file does not exist" : "Üst veri dosyası bulunamadı",
    "Cannot delete metadata" : "Üst veri silinemedi",
    "You are not allowed to create the lock" : "Kilit oluşturma izniniz yok",
    "File already locked" : "Dosya zaten kilitlenmiş",
    "Cannot update filedrop" : "Dosya bırakma güncellenemedi",
    "You are not allowed to lock the root" : "Root kullanıcısını kilitleme izniniz yok",
    "You are not allowed to remove the lock" : "Kilidi açma izniniz yok",
    "File not locked" : "Dosya kilitli değil",
    "You are not allowed to edit the file, make sure to first lock it, and then send the right token" : "Dosyayı düzenleme izniniz yok, önce dosyayı kilitlediğinizden emin olun ve sonra doğru kodu gönderin",
    "Only the owner can delete the metadata-file" : "Üst veri dosyasını yalnızca sahibi silebilir",
    "You are not allowed to edit the metadata of this folder" : "Bu klasörün üst verilerini düzenlemenize izin verilmiyor",
    "Encrypted share" : "Şifrelenmiş paylaşım",
    "End-to-End Encryption" : "Uçtan uca şifreleme",
    "Enable client-side end-to-end encryption for your most sensitive files, ensuring complete privacy with zero-knowledge security." : "En önemli dosyalarınız için istemci tarafında uçtan uca şifrelemeyi açın, sıfır bilgi güvenliği ile tam gizlilik sağlayın.",
    "## **End-to-End Encryption**\n\n### For End Users\n\n**Protect your most sensitive files with strong encryption.**\n\nThe End-to-End Encryption app gives you complete control over your data privacy.\nWith this app, you can encrypt specific folders so that only you (and those you trust) can access their contents.\nYour files are encrypted on your device before they reach the server, ensuring that no one—not even the server administrator—can read them.\n\n**Benefits:**\n- 🔒 **True privacy**: Files are encrypted on your device and can only be decrypted by you\n- 📱 **Works across all platforms**: Fully supported on desktop, Android, iOS clients, and as you wish even in the browser\n- 🎯 **Selective encryption**: Choose which folders to encrypt\n- 🛡️ **Secure sharing**: Share encrypted files with other users or even secure public upload using the encrypted file drop\n\n---\n\n### For Administrators\n\n**Enterprise-ready end-to-end encryption infrastructure for your Nextcloud instance.**\n\nThis app provides all the necessary server-side APIs and infrastructure to enable End-to-End encryption (E2EE) for your users.\nIt ensures that encrypted data remains secure throughout its lifecycle on your server.\n\n**Technical highlights:**\n- 🔐 **Complete API suite**: Provides all client-side APIs needed for E2EE implementation\n- 🔒 **Secure FileDrop integration**: Enables secure file sharing with encryption\n- 🛡️ **Zero-knowledge architecture**: Server never has access to encryption keys\n- ⚙️ **Group restrictions**: Limit app usage to specific user groups if needed\n- 🔄 **Background job management**: Automatic rollback handling for failed operations\n\n### Setup\nThis application provides the server-side infrastructure for end-to-end encryption, but it requires client support to function.\nTo enable end-to-end encryption, users will need to install the corresponding client-side app on their devices (desktop, Android, iOS) or use the web client.\n\nUsing the web interface, after enabling it in the personal settings, allows you to encrypt files and folders directly in the browser,\nproviding a seamless experience without needing additional software. But also requires some kind of trust in the server as the code is delivered by the server and could be manipulated.\n\nOnce enable through clients or the web interface, you can create encrypted folders and upload or move files into them.\nThe clients and the web interface will handle the encryption and decryption processes automatically.\n\n⚠️ This comes with some limitations and caveats, as only normal file operations can be handled.\nMeaning that some apps in the web interface do not work with encrypted files." : "## **Uçtan uca şifreleme**\n\n### Son kullanıcılar için\n\n**En önemli dosyalarınızı güçlü şifreleme ile koruyun. **\n\nUçtan Uca Şifreleme uygulaması, veri gizliliğiniz üzerinde tam denetim sağlar.\nBu uygulama ile belirli klasörleri şifreleyebilirsiniz, böylece yalnızca siz (ve güvendiğiniz kişiler) içeriklerine erişebilir.\nDosyalarınız sunucuya ulaşmadan önce aygıtınızda şifrelenir ve hiç kimse - sunucu yöneticisi bile - onları okuyamaz.\n\n**Yararları:**\n- 🔒 **Gerçek gizlilik**: Dosyalar aygıtınızda şifrelenir ve şifresi yalnızca sizin tarafınızdan çözülebilir\n- 📱 **Tüm platformlarda çalışır**: Bilgisayar, Android, iOS istemcilerinde ve tarayıcıda istediğiniz gibi tümüyle desteklenir\n- 🎯 **Seçici şifreleme**: Hangi klasörlerin şifreleneceğini seçebilirsiniz\n- 🛡️ **Güvenli paylaşım**: Şifrelenmiş dosyaları diğer kullanıcılarla ve şifrelenmiş FileDrop ile herkese açık olarak güvenle paylaşabilirsiniz \n\n---\n\n### Yöneticiler için\n\n**Nextcloud kopyanız için kuruluşunuzda hazır uçtan uca şifreleme altyapısı. **\n\nBu uygulama, kullanıcılarınız için Uçtan Uca şifreleme (E2EE) kullanması için sunucu tarafında gerekli tüm API uygulamalarını ve altyapıyı sağlar.\nŞifrelenmiş verilerin sunucunuzdaki yaşam döngüsü boyunca güvenli kalmasını sağlar.\n\n**Öne çıkan teknik özellikler:**\n- 🔐 **Tam API paketi**: Uçtan uca şifreleme uygulaması için istemci tarafında gereken tüm API uygulamalarını sağlar\n- 🔒 **Güvenli FileDrop bütünleştirmesi**: Şifreleme ile güvenli dosya paylaşımını sağlar\n- 🛡️ **Sıfır bilgi mimarisi**: Sunucu şifreleme anahtarlarına hiçbir zaman erişemez\n- ⚙️ **Grup kısıtlamaları**: Uygulamanın kullanımı gerektiğinde belirli kullanıcı gruplarıyla sınırlanabilir\n- 🔄 **Arka plan iş yönetimi**: Tamamlanamayan işlemleri otomatik olarak geri alma\n\n### Kurulum\nBu uygulama, uçtan uca şifreleme için sunucu tarafındaki altyapıyı sağlar, ancak çalışması için istemci desteği gereklidir.\nUçtan uca şifrelemeyi açmak için kullanıcıların ilgili istemci tarafı uygulamasını aygıtlarına (bilgisayar, Android, iOS) kurmaları veya internet istemcisini kullanmaları gerekir.\n\nİnternet arayüzünü kullanmak, kişisel ayarlardan açıldıktan sonra, dosyaların ve klasörlerin doğrudan tarayıcıda şifrelenmesini sağlar,\nEk yazılım gerekmeden sorunsuz bir deneyim sağlar. Ancak aynı zamanda kod sunucu tarafından teslim edildiği ve değiştirilebileceği için de sunucuya bir şekilde güvenilmesi gerekir.\n\nİstemciler veya internet arayüzü ile açtıktan sonra, şifrelenmiş klasörler oluşturabilir ve dosyaları bunlara yükleyebilir veya taşıyabilirsiniz.\n\nİstemciler ve internet arayüzü, şifreleme ve şifre çözme süreçlerini otomatik olarak yapar.\n\n⚠️ Bu özellik yalnızca normal dosya işlemlerini yapabildiğinden, bazı sınırlamalar ve uyarılar vardır.\nİnternet arayüzündeki bazı uygulamalar şifrelenmiş dosyalarla çalışamaz.",
    "The server could serve malicious source code to extract the secret that protects your files." : "Sunucuda, dosyalarınızı koruyan parolayı öğrenmek için kötü amaçlı bir kod bulunabilir.",
    "I understand the risks" : "Riskleri anladım",
    "Do not show this warning the next time" : "Bu ileti bir daha görüntülenmesin",
    "Create new encrypted folder" : "Yeni şifrelenmiş klasör oluştur",
    "Checking encryption setup …" : "Şifreleme kurulumu denetleniyor…",
    "Submit" : "Gönder",
    "Create folder" : "Klasör oluştur",
    "Folder name cannot be empty" : "Klasör adı boş olamaz",
    "A folder with this name already exists" : "Aynı adlı bir klasör zaten var",
    "\"{char}\" is not allowed in folder names" : "Klasör adlarında \"{char}\" karakteri kullanılamaz",
    "Folder name cannot end with \"{extension}\"" : "Klasör adları \"{extension}\" ile bitemez.",
    "Folder name is invalid" : "Klasör adı geçersiz",
    "Creating folder …" : "Klasör oluşturuluyor…",
    "Folder name" : "Klasör adı",
    "Encrypting your files in the browser can weaken security" : "Dosyalarınızın tarayıcıda şifrelemek güvenliği zayıflatabilir",
    "Decrypting with given mnemonic failed" : "Belirtilen anımsatıcı ile şifre çözülemedi",
    "Mnemonic" : "Anımsatıcı",
    "Setup encryption" : "Şifreleme kurulumu",
    "Continue ({secondsLeft})" : "İlerle ({secondsLeft})",
    "Continue" : "İlerle",
    "To set up encryption, a new private key will be generated for you." : "Şifrelemeyi kurmak için size özel yeni bir kişisel anahtar oluşturulacak.",
    "Please make sure to back up the following recovery phrase, as it will be required to access your encrypted files." : "Şifrelenmiş dosyalarınıza erişmek için gerekeceğinden, aşağıdaki kurtarma ifadesini yedeklediğinizden emin olun.",
    "Your recovery phrase is:" : "Kurtarma ifadeniz:",
    "Download and decrypt" : "İndir ve şifresini çöz",
    "Download and decrypt all selected files." : "Tüm seçilmiş dosyaları indir ve şifrelerini çöz.",
    "Downloading …" : "İndiriliyor…",
    "Failed downloads" : "İndirilemeyenler",
    "Download" : "İndir",
    "End-to-end encrypted link shares" : "Uçtan uca şifrelenmiş bağlantı paylaşımları",
    "Link share" : "Bağlantı paylaşımı",
    "End-to-end encrypted link share" : "Uçtan uca şifrelenmiş bağlantı paylaşımı",
    "Permissions" : "İzinler",
    "Upload only" : "Yalnızca yükleme",
    "View only" : "Yalnızca görüntüleme",
    "View and upload" : "Görüntüle ve yükle",
    "Unlike upload-only shares view-only shares require a trusted server to enforce the restriction." : "Yalnızca yükleme paylaşımlarının aksine, yalnızca görüntüleme paylaşımlarında, kısıtlamayı uygulamak için güvenilen bir sunucu gerekir.",
    "Share url" : "Adresi paylaş",
    "Please share the secret mnemonic with the recipient using a secure second channel." : "Lütfen güvenli bir ikinci kanaldan gizli anımsatıcıyı alıcıyla paylaşın.",
    "Update link share" : "Bağlantı paylaşımını güncelle",
    "Create link share" : "Bağlantı paylaşımı oluştur",
    "Close" : "Kapat",
    "Copy file drop link" : "Dosya bırakma bağlantısını kopyala",
    "Delete file drop" : "Dosya bırakmayı sil",
    "The selected user has not yet enabled end-to-end encryption." : "Seçilmiş kullanıcı henüz uçtan uca şifrelemeyi açmamış.",
    "Share permissions updated successfully." : "Paylaşım izinleri güncellendi.",
    "End-to-end encrypted shares" : "Uçtan uca şifrelenmiş paylaşımlar",
    "Share recipients always have access to the full encrypted folder." : "Paylaşım alıcıları her zaman tam şifrelenmiş klasöre erişebilir.",
    "It is only possible to share with accounts that have already setup end-to-end encryption." : "Yalnızca uçtan uca şifrelemeyi zaten ayarlamış hesaplarla paylaşılabilir.",
    "Create end-to-end encrypted share" : "Uçtan uca şifrelenmiş paylaşım oluştur",
    "Read, write, and share" : "Okuma, yazma ve paylaşma",
    "Read only" : "Salt okunur",
    "Edit" : "Düzenle",
    "Remove" : "Kaldır",
    "User share setup" : "Kullanıcı paylaşımı kurulumu",
    "When limiting the permissions you have to trust the server to enforce the limitation." : "İzinleri sınırlarken, sınırlamayı uygulamak için sunucuya güvenmeniz gerekir.",
    "Share permissions" : "Paylaşım izinleri",
    "Save" : "Kaydet",
    "Enter your 12 words mnemonic" : "12 sözcüklü anımsatıcınızı yazın",
    "Decrypting your files in the browser can weaken security" : "Dosyalarınızın şifresini tarayıcınızda çözmek güvenliği zayıflatabilir",
    "The upload completed, but the file drop is still being processed on the server." : "Yükleme tamamlandı, ancak bırakılan dosyanın sunucuda işlenmesi sürüyor.",
    "Some files failed to upload." : "Bazı dosyalar yüklenemedi.",
    "This share is using a legacy encryption method. Please ask the share owner to update the encryption metadata." : "Bu paylaşım eski bir şifreleme yöntemi kullanıyor. Lütfen paylaşım sahibinden şifreleme üst verilerini güncellemesini isteyin.",
    "Select or drop files" : "Dosyaları seçin ya da sürükleyip bırakın",
    "Uploaded files" : "Yüklenmiş dosyalar",
    "Upload failed" : "Yüklenemedi",
    "Upload successful" : "Yüklendi",
    "Uploading…" : "Yükleniyor…",
    "Upload encrypted files to {fileName}" : "{fileName} üzerine şifrelenmiş dosyaları yükle",
    "Failed to load shares." : "Paylaşımlar yüklenemedi",
    "Loading shares…" : "Paylaşımlar yükleniyor…",
    "Loading metadata…" : "Üst veriler yükleniyor…",
    "Saved groups" : "Kaydedilmiş gruplar",
    "Limit to groups" : "Şu gruplarla sınırlansın",
    "When at least one group is selected, only people of the listed groups can use the End-to-End encryption app." : "En az bir grup seçildiğinde, yalnızca belirtilen gruplardaki kişiler uçtan uca şifreleme uygulamasını kullanabilir.",
    "Limit app usage to groups" : "Uygulama kullanımı şu gruplarla sınırlansın",
    "Confirm resetting keys" : "Anahtarların sıfırlanmasını onaylayın",
    "This is the final warning: Do you really want to reset your keys?" : "Bu son uyarıdır: Anahtarlarınızı sıfırlamak istediğinize emin misiniz?",
    "Cancel" : "İptal",
    "Reset keys" : "Anahtarları sıfırla",
    "End-to-end encryption is currently enabled and correctly setup." : "Uçtan uca şifreleme açık ve doğru şekilde ayarlanmış.",
    "End-to-end encryption is currently disabled. You can set it up with the {productName} clients." : "Uçtan uca şifreleme şu kapalı. {productName} istemcileriyle ayarlayabilirsiniz.",
    "End-to-end encryption keys reset" : "Uçtan uca şifreleme anahtarları sıfırlandı",
    "Unable to reset end-to-end encryption" : "Uçtan uca şifreleme sıfırlanamadı",
    "End-to-end encryption" : "Uçtan uca şifreleme",
    "End to end encryption not available" : "Uçtan uca şifreleme kullanılamıyor",
    "Either your browser does not support end-to-end encryption or you are using an insecure connection (HTTP). Please use a modern browser and ensure you are using HTTPS." : "Tarayıcınız uçtan uca şifrelemeyi desteklemiyor ya da güvenli olmayan bir bağlantı (HTTP) kullanıyorsunuz. Lütfen modern bir tarayıcı ve HTTPS kullandığınızdan emin olun.",
    "Enable E2EE navigation in browser" : "Tarayıcıda E2EE gezinmesi kullanılsın",
    "Enabling E2EE in the browser can weaken security" : "Tarayıcıda E2EE gezinmesinin kullanılması güvenliği zayıflatabilir",
    "Reset end-to-end encryption" : "Uçtan uca şifrelemeyi sıfırla",
    "Please read carefully before resetting your end-to-end encryption keys" : "Lütfen uçtan uca şifreleme anahtarlarınızı sıfırlamadan önce dikkatlice okuyun",
    "Once your end-to-end encryption keys are reset, all files stored in your encrypted folder will be inaccessible." : "Uçtan uca şifreleme anahtarlarınız sıfırlandığında, şifrelenmiş klasörünüzde kayıtlı tüm dosyalar erişilemez olacak.",
    "You should only reset your end-to-end encryption keys if you lost your secure key words (mnemonic)." : "Uçtan uca şifreleme anahtarlarınızı yalnızca güvenli anahtar sözcüklerinizi (anımsatıcı) kaybettiyseniz sıfırlamanız gerekir.",
    "Check on all connected devices if you can retrieve your mnemonic." : "Bağlı tüm aygıtlardan anımsatıcınızı alıp alamadığınızı denetleyin.",
    "Any still connected device might cause problems after deleting the keys, so it is better to disconnect and reconnect the devices again." : "Anahtarlar silindikten sonra bağlı olan aygıtlarla ilgili sorun çıkabilir. Bu nedenle aygıtların bağlantısını kesip yeniden bağlantı kurmak daha iyidir.",
    "Delete existing encrypted files" : "Var olan şifrelenmiş dosyalar silinsin",
    "Preparing download for {file}" : "{file} için indirme hazırlanıyor",
    "Download unencrypted" : "Şifresini çözmeden indir",
    "Sharing options" : "Paylaşım seçenekleri",
    "New encrypted folder" : "Yeni şifrelenmiş klasör",
    "Creating new encrypted folder: Cancelled" : "Yeni şifrelenmiş klasör ekleniyor: İptal edildi",
    "Creating new encrypted folder: Failed" : "Yeni şifrelenmiş klasör ekleniyor: Tamamlanamadı",
    "Decrypting {file}" : "{file} şifresi çözülüyor",
    "An error occurred while decrypting the file. Please try downloading the file again." : "Dosyanın şifresi çözülürken bir sorun çıktı. Lütfen dosyayı yeniden indirmeyi deneyin.",
    "Pending file drop migrations" : "Bekleyen dosya bırakma aktarımları",
    "There are pending file drop migrations that need to be completed. Do you want to complete them now?" : "Tamamlanması gereken bekleyen dosya bırakma aktarımları var. Bunları şimdi tamamlamak ister misiniz?",
    "Migrate now" : "Şimdi aktar",
    "Migrate later" : "Sonra aktar",
    "Migrating file drop entries, please do not close this tab…" : "Dosya bırakma kayıtları aktarılıyor. Lütfen bu sekmeyi kapatmayın...",
    "An error occurred while migrating one file drop entry. Please try again later." : "Bir dosya bırakma kaydı aktarılırken bir sorun çıktı. Lütfen bir süre sonra yeniden deneyin.",
    "There are pending tasks that need to be completed. Are you sure you want to leave?" : "Tamamlanması gereken bekleyen görevler var. Ayrılmak istediğinize emin misiniz?",
    "End-to-end encryption endpoint" : "Uçtan uca şifreleme uç noktası",
    "This app provides all the necessary APIs to implement End-to-End encryption on the client side.\nAdditionally it implements Secure FileDrop and makes sure that End-to-End encrypted files are neither accessible via the web interface nor other WebDAV clients." : "Bu uygulama, istemci tarafında uçtan uca şifreleme kullanabilmek için gereken tüm API uygulamalarını sağlar.\nAyrıca güvenli dosya bırakmayı kullanır ve uçtan uca şifrelenmiş dosyaların internet arayüzü veya diğer WebDAV istemcileri aracılığıyla erişilebilir olmadığından emin olur.",
    "Error while uploading files" : "Dosyalar yüklenirken sorun çıktı",
    "Error while uploading metadata" : "Üst veriler yüklenirken sorun çıktı"
},
"nplurals=2; plural=(n > 1);");
