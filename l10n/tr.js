OC.L10N.register(
    "end_to_end_encryption",
    {
    "This is someone else's private key" : "Bu başka birisinin özel anahtarı",
    "Could not find the private key of the user %s" : "%s kullanıcısının özel anahtarı bulunamadı",
    "Internal error" : "İç hata",
    "You are not allowed to delete this private key" : "Bu özel anahtarı silme izniniz yok",
    "Could not find the private key belonging to the user %s" : "%s kullanıcısına ait özel anahtar bulunamadı",
    "internal error" : "İç hata",
    "Could not find the public key belonging to the user %s" : "%s kullanıcısına ait herkese açık anahtar bulunamadı",
    "Common name (CN) does not match the current user" : "Ortak ad (CN) geçerli kullanıcı ile uyuşmuyor",
    "Could not find the public key belonging to %s" : "%s kullanıcısına ait herkese açık anahtar bulunamadı",
    "This is not your private key to delete" : "Silinecek özel anahtar sizin değil",
    "Could not find metadata for \"%s\"" : "\"%s\" üst verisi bulunamadı",
    "Can\\'t read metadata" : "Üst veri okunamadı",
    "Can\\'t store metadata" : "Üst veri kaydedilemedi",
    "You are not allowed to edit the file, make sure to first lock it, and then send the right token" : "Dosyayı düzenleme izniniz yok, önce dosyayı kilitlediğinizden emin olun ve sonra doğru kodu gönderin",
    "Metadata-file doesn\\'t exist" : "Üst veri dosyası bulunamadı",
    "Only the owner can delete the metadata-file" : "Üst veri dosyasını yalnız sahibi silebilir",
    "Can\\'t delete metadata" : "Üst veri silinemedi",
    "File already locked" : "Dosya zaten kilitlenmiş",
    "You are not allowed to remove the lock" : "Kilidi açma izniniz yok",
    "File not locked" : "Dosya kilitli değil",
    "Can not decode userlist" : "Kullanıcı listesinin kodu çözülemedi",
    "End-to-End Encryption" : "Uçtan Uca Şifreleme",
    "End-to-end encryption endpoint" : "Uçtan uca şifreleme bitiş noktası",
    "Provides the necessary endpoint to enable end-to-end encryption. End-to-end encryption is still in alpha state. Use only with test data, never in production!" : "Kullanıcıların uçtan uca şifrelemeyi açmayı sağlar. Uçtan uca şifreleme hala test aşamasındadır. Sadece test verileri ile kullanın, güncel versiyonda kullanmayın!",
    "Provides the necessary endpoint to enable end-to-end encryption. End-to-end encryption is still in alpha state, don't use this in production and only with test data!" : "Uçtan uca şifreleme için gereken bitiş noktasını sağlar. Uçtan uca şifreleme özelliği henüz geliştirme aşamasında olduğundan üretim sitelerinde kullanmayın ve yalnız deneme verileri ile kulanın."
},
"nplurals=2; plural=(n > 1);");
