OC.L10N.register(
    "end_to_end_encryption",
    {
    "Internal error" : "Earráid inmheánach",
    "You are not allowed to delete this private key" : "Níl cead agat an eochair phríobháideach seo a scriosadh",
    "Could not find the private key belonging to the user %s" : "Níorbh fhéidir an eochair phríobháideach a bhaineann leis an úsáideoir %s a aimsiú",
    "Could not parse the CSR, please make sure to submit a valid CSR" : "Níorbh fhéidir an CSR a pharsáil, déan cinnte CSR bailí a chur isteach le do thoil.",
    "Common name (CN) does not match the share token" : "Ní hionann an t-ainm coitianta (CN) agus an comhartha scaireanna",
    "You are not the owner of the share" : "Ní úinéir na scaire thú",
    "Common name (CN) does not match the current user" : "Ní mheaitseálann an t-ainm coitianta (CN) an t-úsáideoir reatha",
    "Could not find the public key belonging to %s" : "Níorbh fhéidir an eochair phoiblí de %s a aimsiú",
    "This is not your public key to delete" : "Ní hé seo d'eochair phoiblí le scriosadh",
    "Cannot decode userlist" : "Ní féidir an liosta úsáideora a dhíchódú",
    "Could not find metadata for \"%s\"" : "Níorbh fhéidir meiteashonraí a aimsiú le haghaidh \"%s\"",
    "Cannot read metadata" : "Ní féidir meiteashonraí a léamh",
    "Cannot store metadata" : "Ní féidir meiteashonraí a stóráil",
    "Metadata-file does not exist" : "Níl comhad meiteashonraí ann",
    "Cannot delete metadata" : "Ní féidir meiteashonraí a scriosadh",
    "You are not allowed to create the lock" : "Níl cead agat an glas a chruthú",
    "File already locked" : "Comhad faoi ghlas cheana féin",
    "Cannot update filedrop" : "Ní féidir an drop drop a nuashonrú",
    "You are not allowed to lock the root" : "Níl cead agat an fhréamh a ghlasáil",
    "You are not allowed to remove the lock" : "Níl cead agat an glas a bhaint",
    "File not locked" : "Níl an comhad faoi ghlas",
    "You are not allowed to edit the file, make sure to first lock it, and then send the right token" : "Níl cead agat an comhad a chur in eagar, déan cinnte é a ghlasáil ar dtús, agus ansin an comhartha ceart a sheoladh",
    "Only the owner can delete the metadata-file" : "Ní féidir ach leis an úinéir an comhad meiteashonraí a scriosadh",
    "You are not allowed to edit the metadata of this folder" : "Níl cead agat meiteashonraí an fhillteáin seo a chur in eagar",
    "Encrypted share" : "Roinn criptithe",
    "End-to-End Encryption" : "Criptiú Ceann go Deireadh",
    "Enable client-side end-to-end encryption for your most sensitive files, ensuring complete privacy with zero-knowledge security." : "Cumasaigh criptiú foirceann go foirceann taobh an chliaint do do chuid comhad is íogaire, ag cinntiú príobháideachta iomlán le slándáil nialasach eolais.",
    "## **End-to-End Encryption**\n\n### For End Users\n\n**Protect your most sensitive files with strong encryption.**\n\nThe End-to-End Encryption app gives you complete control over your data privacy.\nWith this app, you can encrypt specific folders so that only you (and those you trust) can access their contents.\nYour files are encrypted on your device before they reach the server, ensuring that no one—not even the server administrator—can read them.\n\n**Benefits:**\n- 🔒 **True privacy**: Files are encrypted on your device and can only be decrypted by you\n- 📱 **Works across all platforms**: Fully supported on desktop, Android, iOS clients, and as you wish even in the browser\n- 🎯 **Selective encryption**: Choose which folders to encrypt\n- 🛡️ **Secure sharing**: Share encrypted files with other users or even secure public upload using the encrypted file drop\n\n---\n\n### For Administrators\n\n**Enterprise-ready end-to-end encryption infrastructure for your Nextcloud instance.**\n\nThis app provides all the necessary server-side APIs and infrastructure to enable End-to-End encryption (E2EE) for your users.\nIt ensures that encrypted data remains secure throughout its lifecycle on your server.\n\n**Technical highlights:**\n- 🔐 **Complete API suite**: Provides all client-side APIs needed for E2EE implementation\n- 🔒 **Secure FileDrop integration**: Enables secure file sharing with encryption\n- 🛡️ **Zero-knowledge architecture**: Server never has access to encryption keys\n- ⚙️ **Group restrictions**: Limit app usage to specific user groups if needed\n- 🔄 **Background job management**: Automatic rollback handling for failed operations\n\n### Setup\nThis application provides the server-side infrastructure for end-to-end encryption, but it requires client support to function.\nTo enable end-to-end encryption, users will need to install the corresponding client-side app on their devices (desktop, Android, iOS) or use the web client.\n\nUsing the web interface, after enabling it in the personal settings, allows you to encrypt files and folders directly in the browser,\nproviding a seamless experience without needing additional software. But also requires some kind of trust in the server as the code is delivered by the server and could be manipulated.\n\nOnce enable through clients or the web interface, you can create encrypted folders and upload or move files into them.\nThe clients and the web interface will handle the encryption and decryption processes automatically.\n\n⚠️ This comes with some limitations and caveats, as only normal file operations can be handled.\nMeaning that some apps in the web interface do not work with encrypted files." : "## **Criptiú Deireadh go Deireadh**\n\n### Do Úsáideoirí Deiridh\n\n**Cosain do chuid comhad is íogaire le criptiú láidir.**\n\nTugann an aip Criptiú Deireadh go Deireadh smacht iomlán duit ar do phríobháideacht sonraí.\nLeis an aip seo, is féidir leat fillteáin shonracha a chriptiú ionas nach féidir ach leatsa (agus leo siúd a bhfuil muinín agat astu) rochtain a fháil ar a n-ábhar.\n\nCriptítear do chuid comhad ar do ghléas sula sroicheann siad an freastalaí, rud a chinntíonn nach féidir le duine ar bith—fiú riarthóir an fhreastalaí—iad a léamh.\n\n**Sochair:**\n- 🔒 **Príobháideacht fhíor**: Criptítear comhaid ar do ghléas agus ní féidir ach leatsa iad a dhíchriptiú\n- 📱 **Oibríonn sé ar fud gach ardáin**: Tacaíocht iomlán ar chliaint deisce, Android, iOS, agus de réir mar is mian leat fiú sa bhrabhsálaí\n- 🎯 **Criptiú roghnach**: Roghnaigh cé na fillteáin le criptiú\n- 🛡️ **Comhroinnt shlán**: Comhroinn comhaid chriptithe le húsáideoirí eile nó fiú uaslódáil phoiblí shlán ag baint úsáide as an titim comhad criptithe\n\n---\n\n### Do Riarthóirí\n\n**Bonneagar criptithe ó cheann go ceann atá réidh le haghaidh fiontraíochta do do chás Nextcloud.**\n\nSoláthraíonn an aip seo na APIanna agus an bonneagar taobh an fhreastalaí riachtanach go léir chun criptiú ó cheann go ceann (E2EE) a chumasú do d'úsáideoirí.\n\nCinntíonn sé go bhfanann sonraí criptithe slán ar feadh a saolré ar do fhreastalaí.\n\n**Buaicphointí teicniúla:**\n- 🔐 **Sraith iomlán API**: Soláthraíonn sé gach API taobh cliaint atá riachtanach le haghaidh cur i bhfeidhm E2EE\n- 🔒 **Comhtháthú slán FileDrop**: Cumasaíonn sé comhroinnt comhad slán le criptiú\n- 🛡️ **Ailtireacht nialasach eolais**: Ní bhíonn rochtain ag an bhfreastalaí ar eochracha criptithe riamh\n- ⚙️ **Srianta grúpa**: Teorainn a chur le húsáid aipeanna do ghrúpaí úsáideoirí sonracha más gá\n- 🔄 **Bainistíocht poist chúlra**: Láimhseáil uathoibríoch rolladh ar ais le haghaidh oibríochtaí nár éirigh leo\n\n### Socrú\nSoláthraíonn an feidhmchlár seo an bonneagar taobh freastalaí le haghaidh criptithe ó cheann ceann go ceann, ach teastaíonn tacaíocht chliaint uaidh chun go bhfeidhmeoidh sé.\nChun criptiú ó cheann ceann go ceann a chumasú, beidh ar úsáideoirí an aip chomhfhreagrach taobh cliaint a shuiteáil ar a ngléasanna (deasc, Android, iOS) nó an cliant gréasáin a úsáid.\n\nTrí úsáid a bhaint as an gcomhéadan gréasáin, tar éis é a chumasú sna socruithe pearsanta, is féidir leat comhaid agus fillteáin a chriptiú go díreach sa bhrabhsálaí,\nag soláthar eispéireas gan uaim gan gá le bogearraí breise. Ach teastaíonn cineál éigin muiníne sa fhreastalaí freisin mar go seachadann an freastalaí an cód agus d'fhéadfaí é a ionramháil.\n\nNuair a bheidh sé cumasaithe trí chliaint nó an comhéadan gréasáin, is féidir leat fillteáin chriptithe a chruthú agus comhaid a uaslódáil nó a bhogadh isteach iontu.\n\nLáimhseálfaidh na cliaint agus an comhéadan gréasáin na próisis chriptithe agus dhíchriptithe go huathoibríoch.\n\n⚠️ Tagann roinnt teorainneacha agus rabhaidh leis seo, mar ní féidir ach oibríochtaí comhaid gnáth a láimhseáil.\n\nCiallaíonn sé sin nach n-oibríonn roinnt aipeanna sa chomhéadan gréasáin le comhaid chriptithe.",
    "The server could serve malicious source code to extract the secret that protects your files." : "D'fhéadfadh an freastalaí cód foinse mailíseach a sheirbheáil chun an rún a chosnaíonn do chuid comhad a bhaint amach.",
    "I understand the risks" : "Tuigim na rioscaí",
    "Do not show this warning the next time" : "Ná taispeáin an rabhadh seo an chéad uair eile",
    "Create new encrypted folder" : "Cruthaigh fillteán criptithe nua",
    "Checking encryption setup …" : "Ag seiceáil socrú criptithe …",
    "Submit" : "Cuir isteach",
    "Create folder" : "Cruthaigh fillteán",
    "Folder name cannot be empty" : "Ní féidir ainm fillteáin a bheith folamh",
    "A folder with this name already exists" : "Tá fillteán leis an ainm seo ann cheana féin",
    "\"{char}\" is not allowed in folder names" : "Ní cheadaítear \"{char}\" in ainmneacha fillteán",
    "Folder name cannot end with \"{extension}\"" : "Ní féidir ainm fillteáin a chríochnú le \"{extension}\"",
    "Folder name is invalid" : "Tá ainm an fhillteáin neamhbhailí",
    "Creating folder …" : "Ag cruthú fillteáin …",
    "Folder name" : "Ainm fillteáin",
    "Encrypting your files in the browser can weaken security" : "Is féidir le criptiú do chuid comhad sa bhrabhsálaí an tslándáil a lagú",
    "Decrypting with given mnemonic failed" : "Theip ar dhíchriptiú leis an mnemonic tugtha",
    "Mnemonic" : "Cuimhneamh",
    "Setup encryption" : "Socraigh criptiú",
    "Continue ({secondsLeft})" : "Lean ar aghaidh ({secondsLeft})",
    "Continue" : "Lean ar aghaidh",
    "To set up encryption, a new private key will be generated for you." : "Chun criptiú a shocrú, ginfear eochair phríobháideach nua duit.",
    "Please make sure to back up the following recovery phrase, as it will be required to access your encrypted files." : "Déan cinnte cúltaca den fhrása aisghabhála seo a leanas a dhéanamh, mar beidh sé ag teastáil chun rochtain a fháil ar do chuid comhad criptithe.",
    "Your recovery phrase is:" : "Seo do fhrása téarnaimh:",
    "Download and decrypt" : "Íoslódáil agus díchriptigh",
    "Download and decrypt all selected files." : "Íoslódáil agus díchriptigh na comhaid roghnaithe go léir.",
    "Downloading …" : "Ag íoslódáil …",
    "Failed downloads" : "Íoslódálacha nár éirigh leo",
    "Download" : "Íoslódáil",
    "End-to-end encrypted link shares" : "Comhroinnt nasc criptithe ó cheann ceann go ceann",
    "Link share" : "Comhroinnt nasc",
    "End-to-end encrypted link share" : "Comhroinnt nasc criptithe ó cheann ceann go ceann",
    "Permissions" : "Ceadanna",
    "Upload only" : "Uaslódáil ach amháin",
    "View only" : "Amharc amháin",
    "View and upload" : "Féach agus uaslódáil",
    "Unlike upload-only shares view-only shares require a trusted server to enforce the restriction." : "Murab ionann agus scaireanna uaslódála amháin, ní mór freastalaí iontaofa a bheith ag teastáil ó scaireanna amhairc amháin chun an srian a fhorfheidhmiú.",
    "Share url" : "Comhroinn URL",
    "Please share the secret mnemonic with the recipient using a secure second channel." : "Roinn an mnemonic rúnda leis an bhfaighteoir ag baint úsáide as dara cainéal slán, le do thoil.",
    "Update link share" : "Nuashonraigh comhroinnt nasc",
    "Create link share" : "Cruthaigh comhroinnt nasc",
    "Close" : "Dún",
    "Copy file drop link" : "Cóipeáil nasc titim comhaid",
    "Delete file drop" : "Scrios an comhad titim",
    "The selected user has not yet enabled end-to-end encryption." : "Níl criptiú foirceann go foirceann cumasaithe ag an úsáideoir roghnaithe go fóill.",
    "Share permissions updated successfully." : "Nuashonraíodh na ceadanna comhroinnte go rathúil.",
    "End-to-end encrypted shares" : "Scaireanna criptithe ó cheann ceann go ceann",
    "Share recipients always have access to the full encrypted folder." : "Bíonn rochtain ag faighteoirí comhroinnte ar an bhfillteán criptithe iomlán i gcónaí.",
    "It is only possible to share with accounts that have already setup end-to-end encryption." : "Ní féidir comhroinnt ach le cuntais a bhfuil criptiú foirceann go foirceann socraithe acu cheana féin.",
    "Create end-to-end encrypted share" : "Cruthaigh comhroinnt chriptithe ó cheann ceann go ceann",
    "Read, write, and share" : "Léigh, scríobh, agus roinn",
    "Read only" : "Léamh amháin",
    "Edit" : "Cuir in eagar",
    "Remove" : "Bain",
    "User share setup" : "Socrú comhroinnte úsáideora",
    "When limiting the permissions you have to trust the server to enforce the limitation." : "Agus tú ag cur teorainn leis na ceadanna, caithfidh tú muinín a bheith agat as an bhfreastalaí chun an teorainn a fhorfheidhmiú.",
    "Share permissions" : "Ceadanna a roinnt",
    "Save" : "Sábháil",
    "Enter your 12 words mnemonic" : "Cuir isteach do 12 focal cuimhneacháin",
    "Decrypting your files in the browser can weaken security" : "Má dhéantar do chuid comhad a dhíchriptiú sa bhrabhsálaí is féidir an tslándáil a lagú",
    "The upload completed, but the file drop is still being processed on the server." : "Tá an uaslódáil críochnaithe, ach tá an comhad fós á phróiseáil ar an bhfreastalaí.",
    "Some files failed to upload." : "Theip ar roinnt comhad a uaslódáil.",
    "This share is using a legacy encryption method. Please ask the share owner to update the encryption metadata." : "Tá an comhroinnt seo ag baint úsáide as modh criptithe oidhreachta. Iarr ar úinéir na comhroinnte na meiteashonraí criptithe a nuashonrú.",
    "Select or drop files" : "Roghnaigh nó scaoil comhaid",
    "Uploaded files" : "Comhaid uaslódáilte",
    "Upload failed" : "Theip ar an uaslódáil",
    "Upload successful" : "Uaslódáil rathúil",
    "Uploading…" : "Á uaslódáil…",
    "Upload encrypted files to {fileName}" : "Uaslódáil comhaid chriptithe go {fileName}",
    "Failed to load shares." : "Theip ar scaireanna a lódáil.",
    "Loading shares…" : "Ag lódáil scaireanna…",
    "Loading metadata…" : "Meiteashonraí á lódáil…",
    "Saved groups" : "Grúpaí sábháilte",
    "Limit to groups" : "Teorainn do ghrúpaí",
    "When at least one group is selected, only people of the listed groups can use the End-to-End encryption app." : "Nuair a roghnaítear grúpa amháin ar a laghad, ní féidir ach le daoine de na grúpaí liostaithe úsáid a bhaint as an aip criptithe ó cheann go ceann.",
    "Limit app usage to groups" : "Teorainn úsáid aipeanna go grúpaí",
    "Confirm resetting keys" : "Deimhnigh na heochracha a athshocrú",
    "This is the final warning: Do you really want to reset your keys?" : "Is é seo an rabhadh deiridh: Ar mhaith leat i ndáiríre a athshocrú do eochracha?",
    "Cancel" : "Cealaigh",
    "Reset keys" : "Eochracha a athshocrú",
    "End-to-end encryption is currently enabled and correctly setup." : "Tá criptiú ceann go ceann cumasaithe faoi láthair agus socraithe i gceart.",
    "End-to-end encryption is currently disabled. You can set it up with the {productName} clients." : "Tá criptiú ceann go ceann díchumasaithe faoi láthair. Is féidir leat é a shocrú leis na cliaint {productName}.",
    "End-to-end encryption keys reset" : "Eochracha criptithe ceann go deireadh a athshocrú",
    "Unable to reset end-to-end encryption" : "Ní féidir criptiú ceann go ceann a athshocrú",
    "End-to-end encryption" : "Criptiú ceann go deireadh",
    "End to end encryption not available" : "Níl criptiú ó cheann ceann ar fáil",
    "Either your browser does not support end-to-end encryption or you are using an insecure connection (HTTP). Please use a modern browser and ensure you are using HTTPS." : "Ní thacaíonn do bhrabhsálaí le criptiú foirceann go foirceann nó tá nasc neamhshábháilte (HTTP) in úsáid agat. Bain úsáid as brabhsálaí nua-aimseartha agus cinntigh go bhfuil tú ag úsáid HTTPS.",
    "Enable E2EE navigation in browser" : "Cumasaigh nascleanúint E2EE sa bhrabhsálaí",
    "Enabling E2EE in the browser can weaken security" : "Má chuirtear E2EE ar chumas an bhrabhsálaí is féidir an tslándáil a lagú",
    "Reset end-to-end encryption" : "Athshocraigh criptiú ceann go ceann",
    "Please read carefully before resetting your end-to-end encryption keys" : "Léigh go cúramach le do thoil sula athshocraíonn tú d’eochracha criptithe ceann go ceann",
    "Once your end-to-end encryption keys are reset, all files stored in your encrypted folder will be inaccessible." : "Nuair a athshocraítear d’eochracha criptithe ó cheann ceann go ceann, beidh gach comhad atá stóráilte i d’fhillteán criptithe dorochtana.",
    "You should only reset your end-to-end encryption keys if you lost your secure key words (mnemonic)." : "Níor cheart duit d’eochracha criptithe ceann go ceann a athshocrú ach amháin má chaill tú d’eochairfhocail slán (mnemonic).",
    "Check on all connected devices if you can retrieve your mnemonic." : "Seiceáil ar gach gléas nasctha an féidir leat do chuimhneachán a aisghabháil.",
    "Any still connected device might cause problems after deleting the keys, so it is better to disconnect and reconnect the devices again." : "D'fhéadfadh fadhbanna a bheith ag baint le haon ghléas atá fós nasctha tar éis na heochracha a scriosadh, mar sin is fearr na gléasanna a dhínascadh agus a athcheangal arís.",
    "Delete existing encrypted files" : "Scrios comhaid criptithe atá ann cheana féin",
    "Preparing download for {file}" : "Ag ullmhú íoslódáil do {file}",
    "Download unencrypted" : "Íosluchtaigh gan criptithe",
    "Sharing options" : "Roghanna comhroinnte",
    "New encrypted folder" : "Fillteán criptithe nua",
    "Creating new encrypted folder: Cancelled" : "Ag cruthú fillteáin chriptithe nua: Cealaithe",
    "Creating new encrypted folder: Failed" : "Ag cruthú fillteán criptithe nua: Theip air",
    "Decrypting {file}" : "Ag díchriptiú {file}",
    "An error occurred while decrypting the file. Please try downloading the file again." : "Tharla earráid agus an comhad á dhíchriptiú. Déan iarracht an comhad a íoslódáil arís.",
    "Pending file drop migrations" : "Imircí titim comhad ar feitheamh",
    "There are pending file drop migrations that need to be completed. Do you want to complete them now?" : "Tá imirce comhad titime ar feitheamh nach mór a chríochnú. Ar mhaith leat iad a chríochnú anois?",
    "Migrate now" : "Imirce anois",
    "Migrate later" : "Imirce níos déanaí",
    "Migrating file drop entries, please do not close this tab…" : "Ag aistriú iontrálacha titim comhaid, ná dún an cluaisín seo le do thoil…",
    "An error occurred while migrating one file drop entry. Please try again later." : "Tharla earráid agus iontráil amháin titim comhaid á haistriú. Déan iarracht arís ar ball.",
    "There are pending tasks that need to be completed. Are you sure you want to leave?" : "Tá tascanna ar feitheamh nach mór a chríochnú. An bhfuil tú cinnte gur mian leat imeacht?",
    "End-to-end encryption endpoint" : "Críochphointe criptithe ceann go ceann",
    "This app provides all the necessary APIs to implement End-to-End encryption on the client side.\nAdditionally it implements Secure FileDrop and makes sure that End-to-End encrypted files are neither accessible via the web interface nor other WebDAV clients." : "Soláthraíonn an aip seo na APIanna riachtanacha go léir chun criptiú Deireadh go Deireadh a chur i bhfeidhm ar thaobh an chliaint.\nIna theannta sin cuireann sé Secure FileDrop i bhfeidhm agus cinntíonn sé nach bhfuil rochtain ar chomhaid chriptithe ó cheann ceann go ceann tríd an gcomhéadan gréasáin ná trí chliaint WebDAV eile.",
    "Error while uploading files" : "Earráid agus comhaid á n-uaslódáil",
    "Error while uploading metadata" : "Earráid agus meiteashonraí á n-uaslódáil"
},
"nplurals=5; plural=(n==1 ? 0 : n==2 ? 1 : n<7 ? 2 : n<11 ? 3 : 4);");
