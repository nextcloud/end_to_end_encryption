...
class RollbackServiceV1Test extends TestCase {
	...
	private ?\OCA\EndToEndEncryption\RollbackServiceV1 $rollbackService = null;
	...
	public function testRollbackOlderThan(): void {
		...
		$this->fileService->expects($this->exactly(3))
			...
			->willReturnCallback(fn (Folder $folder): bool => match ($folder) {
		...
		$this->metaDataStorage->expects($this->exactly(2))
			...
			->willReturnCallback(function (string $userId, int $fileId): void {
				match ([$userId, $fileId]) {
		...
		$this->lockMapper->expects($this->exactly(3))
			->method('delete')
			->willReturnCallback(function (Lock $lock) use (&$deletedLocks): Lock {
				$deletedLocks[] = $lock;
				return $lock;
			});
		...
		$this->assertEquals([$locks[0], $locks[5], $locks[6]], $deletedLocks);
	}
