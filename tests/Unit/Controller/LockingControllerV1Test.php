...
class LockingControllerV1Test extends TestCase {
	 ...
	 */
	private function mockUserFolder(Folder $node, int $userFolderId = 1): void {
	...
	public function testLockFolderRoot(): void {
		$this->request->method('getParam')
			->with('e2e-token', '')
			->willReturn('sendE2EToken');
		$this->mockUserFolder($this->createMock(Folder::class), self::FILE_ID);
	...
	/**
	 * The permissions of a share are checked against the share token.
	 */
	public function testLockFolderWithShareToken(): void {
		...
		$this->accessManager->expects($this->once())
			...
			->with(self::FILE_ID, true, 'share-token');

		$this->lockManager->expects($this->once())
			->method('lockFile')
			->willReturn('new-token');

		$response = $this->controller->lockFolder(self::FILE_ID, 'share-token');
		$this->assertEquals(['e2e-token' => 'new-token'], $response->getData());
	}
	...
	public function invalidLockDataProvider(): array {
		return [
