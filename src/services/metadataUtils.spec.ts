...
import { decryptMetadataInfo } from './metadataUtils.ts'
import { Metadata, PrivateKeyInfo } from '../models.ts'
...
const mnemonic = 'farm toilet escape army regular funny board consider same leaf fiction spray'

test('Metadata is correctly decrypted', async () => {
	const privateKey = await decryptPrivateKey(privateKeyInfo, mnemonic)
	const metadataInfo = await decryptMetadataInfo(metadata, 'admin', privateKey)
